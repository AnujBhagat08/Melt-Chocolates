...
const Footer = () => {
  ...
  useGSAP(
    () => {
      ...
      // 2. Magnetic Link Effect (Subtle X-axis shift on hover)
      const links = gsap.utils.toArray(".hover-link");
      links.forEach((link) => {
        link.addEventListener("mouseenter", () =>
          gsap.to(link, { x: 10, color: "#3d2b1f", duration: 0.3 }),
        );
        link.addEventListener("mouseleave", () =>
          gsap.to(link, { x: 0, color: "white", duration: 0.3 }),
        );
      });
    },
    { scope: footerRef },
  ...
  const handleSubscribe = (e) => {
    ...
    if (email) {
      ...
      // Kinetic Animation for Subscribe
  ...
  return (
    ...
    >
      ...
      <div className="pt-24 grid grid-cols-1 md:grid-cols-12 gap-12 border-b border-white/10 pb-20">
        ...
        {/* EXPLORE LINKS */}
        <div className="footer-fade col-span-6 md:col-span-2">
          <h3 className="text-sm font-bold tracking-[0.3em] uppercase opacity-40 mb-8">
            Explore
          </h3>
          <ul className="flex flex-col gap-4 text-xl font-medium">
            <li>
              <Link to="/" className="hover-link inline-block">
                Flavors
              </Link>
            </li>
            <li>
              <Link to="/our-story" className="hover-link inline-block">
                Our Story
              </Link>
            </li>
            <li>
              <Link to="/how-its-made" className="hover-link inline-block">
                How it’s Made
              </Link>
            </li>
            <li>
              <Link to="/gift" className="hover-link flex items-center gap-2">
                Gifting{" "}
                <img
                  src={Gift}
                  alt="Gift"
                  className="w-10 transform -rotate-12"
                />
              </Link>
            </li>
          </ul>
        ...
        <div className="footer-fade col-span-6 md:col-span-2">
          ...
          <ul className="flex flex-col gap-4 text-xl font-medium">
            <li>
              <a href="#journey" className="hover-link inline-block">
                Taste Journey
              </a>
            </li>
            <li>
              <a href="#texture" className="hover-link inline-block">
                Texture & Melt
              </a>
            </li>
            <li>
              <a href="#ingredients" className="hover-link inline-block">
                Ingredients
              </a>
            </li>
            <li>
              <a href="#process" className="hover-link inline-block">
                Process
              </a>
            </li>
          </ul>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center py-16 gap-10 footer-fade">
        ...
        <div className="w-full max-w-md">
          {isSubscribed ? (
            <p className="text-2xl font-serif italic text-white animate-pulse">
            ...
            <form onSubmit={handleSubscribe} className="relative group">
              <input
                ...
                className="w-full bg-transparent border-b-2 border-white/20 py-4 text-2xl font-light outline-none focus:border-white transition-all placeholder:text-white/30"
                required
              />
              <button
                type="submit"
                className="newsletter-btn absolute right-0 bottom-4 text-white font-bold uppercase tracking-widest text-xs hover:translate-x-2 transition-transform"
              >
                Join →
              </button>
            </form>
      ...
      <div className="py-8 flex flex-col md:flex-row justify-between items-center text-[10px] uppercase tracking-[0.2em] font-black text-white border-t border-white/20 opacity-100">
        <div>
          Concept by{" "}
          <span className="underline cursor-pointer hover:text-brown transition-colors duration-300">
            MELT
          </span>
        </div>
